        ...
        */
        ...
        public MySettings Load(string FileName)
        {
            try
            {
                ...
                settings.MainForm = MainForm;  // Set MainForm reference immediately
                ...
                {
                    // Settings file doesn't exist - first run or missing file
                    if (MainForm != null)
                    {
                        MainForm.DisplayText("Settings file not found: " + FileName, KnownColor.DarkOrange, true);
                        ...
                        {
                            MessageBox.Show(
                                "Program will now exit. Please run the settings transfer program first.\n\n" +
                                "You can download it from https://www.liteplacer.com/downloads/ .",
                                "Exiting",
                                MessageBoxButtons.OK);
                               // Environment.Exit(0);
                        }
                    }
                    // Return the default settings object that was already created
                    settings.MainForm = MainForm;
                    return settings;
                }
            }
