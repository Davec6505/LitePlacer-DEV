...
#### Change 35: GotoNextPartByMeasurement_m - Thread-Safe Grid Access
**Lines:** 773-872  
**Function:** `GotoNextPartByMeasurement_m(int TapeNumber, out double HoleX, out double HoleY)`  
**Change:** Added InvokeRequired check and thread marshaling for all DataGridView reads
```csharp
...
{
    // Called from UI thread - read directly (existing behavior preserved)
    if (Grid.Rows[TapeNumber].Cells["VerifyHoleWithCamera_Column"].Value != null)
    {
        bool.TryParse(Grid.Rows[TapeNumber].Cells["VerifyHoleWithCamera_Column"].Value.ToString(), 
            out verifyHoleWithCamera);
...
// Rest of function uses ONLY local variables (no more Grid access)
```
**Description:** Prevents cross-thread exceptions when placement operations run on background threads. Reads all DataGridView data (VerifyHoleWithCamera checkbox, Next_X, Next_Y coordinates, tape ID) at function start with proper thread marshaling. Uses synchronous Invoke() to block until UI data is retrieved, then uses local variables for remainder of function. Consistent with NozzlePullTapeIndex_m() thread-safety pattern. Backwards compatible - preserves original behavior when called from UI thread.

---

#### Change 36: GotoNextPartByMeasurement_m - Remove Duplicate Grid Reads
**Lines:** 898-903  
**Function:** `GotoNextPartByMeasurement_m(int TapeNumber, out double HoleX, out double HoleY)`  
**Change:** Removed duplicate Grid reads for NextX and NextY
```csharp
...
```
**Description:** Removed lines 897-917 which were reading Next_X and Next_Y from Grid again. These values are now read once at function start (Change 35) using thread-safe marshaling. Eliminates redundant Grid access and potential cross-thread issues. Uses local variables populated at function start.
